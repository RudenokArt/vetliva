                ...
                <div class='row ts-mx-0'>
                    <? if ($arData["MAIN_BLOCK"]): ?>
                        ...
                        <div class="service-info-container">
                            ...
                            <? endif ?> 
                            <div class="service-desc col-lg-<?= $col_ ?> col-md-<?= $col_ ?>">
                                ...
                                <? if ($arResult['FORSPOTPAYMENT']) : $existsfsptitle = true ?>
                                    <div class="fsp-container"><span class="for-spot-payment for-spot-payment-desc"><?= GetMessage("AVAIL_ON_SPOT") ?></span></div>
                                <? endif ?>
								...
								<div class="service-phrase-description">
								
									<?if($arParams["TYPE"] != "transfers"): ?>
									
										<a <? if ($arResult["SERVICE_POPUP_JS"]) : ?>data-id="<?= $arData["MAIN_BLOCK"]["ID"] ?>"<? endif ?> href="<? if ($arResult["SERVICE_POPUP_JS"]) : ?>#srv-popup-<?= $arData["MAIN_BLOCK"]["ID"] ?><? else: ?>javascript:void(0)<? endif ?>" class="pointer <? if ($arResult["SERVICE_POPUP_JS"]) : ?> open-service-popup <? endif ?>"><?= GetMessage("ITEM_DESCRIPTION") ?></a>
                            ...
                            <div class="col-lg-12 col-md-12">
                                <table class="table rate-table mt-20  ts-d-none ts-d-sm-table">
                                    ...
                                    foreach ($arData["ROWS"] as $key => $arRow): $count++; ?>
                                        <tr <?=$key?> class="<?if ($count>3 && $use_limit):?>hidden<?endif;?>">
                                        ...
                                        <div class="ts-wrap ts-d-block ts-d-sm-none ts-pb-1 ts-my-1 <?if ($count>3 && $use_limit):?>hidden<?endif;?>" style="border: 1px solid lightgrey">
                                            <div class="ts-row">
                                                ...
                                                <div class="ts-col-24 ts-justify-content__center">
                                                    ...
                                                    <? if ($arResult["RATE_POPUP_JS"]): ?><a class="open-rate-popup" data-id="<?= $arRow["ID"] ?>" href="#rate-popup-<?= $key ?>"><?= GetMessage("ABOUT_RATE_TITLE") ?></a> &nbsp&nbsp<? endif ?><? if ($arResult["CANCELLATION_POLICY_POPUP_JS"]): ?><a class="open-cancellation-policy-popup" data-id="<?= $arRow["ID"] ?>" href="#cancellation-policy-popup-<?= $key ?>"><?= GetMessage("CANCELLATION_POLICY_TITLE") ?></a><? endif ?>
                                                    <? if (($arResult['FORSPOTPAYMENT'] || $arRow["FOR_SPOT_PAYMENT"]) && !$existsfsptitle) : ?>
                                                        <span class="for-spot-payment for-spot-payment-mob ts-d-block ts-width-100" style="text-align: center"><?= GetMessage("AVAIL_ON_SPOT") ?></span>
                                                    <?endif?>
													
													 <? if ($arResult['FORSPOTPAYMENT']) : $existsfsptitle = true ?>
														<div class="fsp-container"><span class="for-spot-payment for-spot-payment-mob"><?= GetMessage("AVAIL_ON_SPOT") ?></span></div>
													<? endif ?>
												
												</div>
                                                <div class="ts-col-24 ts-py-2 ts-justify-content__center ts-price" style="text-align: center">
                                                    <?if (isset($arRow["DISCOUNT_PRICE"]) && $arRow["DISCOUNT_PRICE"] > 0):?>
                                                ...
                                                <div class="ts-col-24 ts-justify-content__center">
                                                    <a href="javascript:void(0)" data-add2cart="<?= $arRow["ADD2CART"] ?>"  class="add-to-cart awe-btn awe-btn-1 awe-btn-small">
														<?= GetMessage("BOOK") ?>
													</a>
                                                </div>
                                            </div>
