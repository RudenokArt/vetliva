...
if ($bAllowAccess)
{
    ...
    while ($arProperty = $rsIBLockPropertyList->GetNext())
    {
        ...
        if($arParams["COMPONENT_TEMPLATE"] == "mice") {
            if (in_array($arProperty["ID"], $arParams["PROPERTY_CODES"])) {
                if ($arProperty["CODE"] == "TYPE")
                    $arResult["PROPERTY_LIST"]["LIST_1"][] = $arProperty["ID"];
                elseif ($arProperty["CODE"] == "TYPE_OTHER")
                    $arResult["PROPERTY_LIST"]["LIST_2"][] = $arProperty["ID"];
                elseif ($arProperty["CODE"] == "DATE_FROM" || $arProperty["CODE"] == "DATE_TO")
                    $arResult["PROPERTY_LIST"]["LIST_3"][] = $arProperty["ID"];
                elseif ($arProperty["CODE"] == "CATEGOTY")
                    $arResult["PROPERTY_LIST"]["LIST_4"][] = $arProperty["ID"];
                elseif ($arProperty["CODE"] == "TRANSFER_FROM" || $arProperty["CODE"] == "TRANSFER_TO" || $arProperty["CODE"] == "TRANSFER_DATE_FROM" || $arProperty["CODE"] == "TRANSFER_DATE_TO" || $arProperty["CODE"] == "TRANSFER_PEOPLE")
                    $arResult["PROPERTY_LIST"]["TRANSFER"][] = $arProperty["ID"];
                elseif ($arProperty["CODE"] == "TRANSFER_FROM_OTHER" || $arProperty["CODE"] == "TRANSFER_TO_OTHER")
                    $arResult["PROPERTY_LIST"]["TRANSFER_OTHER"][] = $arProperty["ID"];
                elseif ($arProperty["CODE"] == "PEOPLE")
                    $arResult["PROPERTY_LIST"]["LIST_5"][] = $arProperty["ID"];
                elseif ($arProperty["CODE"] == "MESSAGE")
                    $arResult["PROPERTY_LIST"]["LIST_6"][] = $arProperty["ID"];
                elseif ($arProperty["CODE"] == "NAME" || $arProperty["CODE"] == "COMPANY_NAME")
                    $arResult["PROPERTY_LIST"]["LIST_7"][] = $arProperty["ID"];
                elseif ($arProperty["CODE"] == "PHONE" || $arProperty["CODE"] == "EMAIL")
                    $arResult["PROPERTY_LIST"]["LIST_8"][] = $arProperty["ID"];
                else
            ...
            if (in_array($arProperty["ID"], $arParams["PROPERTY_CODES"])) {
                if ($arProperty["CODE"] == "OA_ARRIVAL_DATE" || $arProperty["CODE"] == "OA_DEPARTURE_DATE" || $arProperty["CODE"] == "OA_NAMBER_DAYS_STAY")
                    $arResult["PROPERTY_LIST"]["LIST_1"][] = $arProperty["ID"];
                elseif ($arProperty["CODE"] == "OA_PLACE_OF_ARRIVAL" || $arProperty["CODE"] == "OA_NAMBER_PERSON_ADL" || $arProperty["CODE"] == "OA_NAMBER_PERSON_CHILD")
                    $arResult["PROPERTY_LIST"]["LIST_2"][] = $arProperty["ID"];
                elseif ($arProperty["CODE"] == "DATE_BIRTH" || $arProperty["CODE"] == "PASSPORT" || $arProperty["CODE"] == "COUNTRY")
                    $arResult["PROPERTY_LIST"]["LIST_1"][] = $arProperty["ID"];
                elseif ($arProperty["CODE"] == "EMAIL" || $arProperty["CODE"] == "PHONE" || $arProperty["CODE"] == "SKYPE")
                    $arResult["PROPERTY_LIST"]["LIST_2"][] = $arProperty["ID"];
                elseif ($arProperty["CODE"] == "ARRIVAL_DATE" || $arProperty["CODE"] == "NAMBER_PERSON" || $arProperty["CODE"] == "NAMBER_DAYS_STAY")
                    $arResult["PROPERTY_LIST"]["LIST_3"][] = $arProperty["ID"];
                else
                    $arResult["PROPERTY_LIST"][$arProperty["CODE"]][] = $arProperty["ID"];
                //$arResult["PROPERTY_LIST"][] = $arProperty["ID"];
    ...
    if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
    {
        ...
        foreach ($arParams["PROPERTY_CODES"]  as $i => $propertyID)
        {
            ...
            if (intval($propertyID) > 0)
            {
                ...
                else
                {
                    ...
                    foreach ($arPropertyValue as $key => $value)
                    {
                        $arFile = $arFilesForSend["PROPERTY_FILE_".$propertyID."_".$key] = $_FILES["PROPERTY_FILE_".$propertyID."_".$key];
						echo "PROPERTY_FILE_".$propertyID."_".$key."</br>";
                        $arFile["del"] = $arFilesForSend["PROPERTY_FILE_".$propertyID."_".$key]["del"] = $_REQUEST["DELETE_FILE"][$propertyID][$key] == "Y" ? "Y" : "";
        ...
        foreach ($arParams["PROPERTY_CODES_REQUIRED"] as $key => $propertyID)
        {
            ...
            if($arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] == "EMAIL"){
                ...
                if(!filter_var($propertyValue, FILTER_VALIDATE_EMAIL)) {
                    $arResult["ERRORS"][] = GetMessage('FORMAT_EMAIL');
                }
        ...
        if (empty($arResult["ERRORS"]))
        {
            ...
            else
            {
                ...
                foreach ($arResult["PROPERTY_LIST_FULL"] as $k=>$idProp){
                    if($idProp["CODE"] == "NAME"){
                        $arUpdateValues["NAME"] = $_REQUEST["PROPERTY"][$k][0].', '.date($DB->DateFormatToPHP(CSite::GetDateFormat("SHORT")));
                    }
                }
                ...
                if (empty($arResult["ERRORS"]) && $arParams["ID"] > 0){
                    ...
                    if(!empty($_REQUEST["PROPERTY"])){
                        foreach ($_REQUEST["PROPERTY"] as $k=>$item) {
                            if($k != "PREVIEW_TEXT" && $arResult["PROPERTY_LIST_FULL"][$k]["CODE"] != "USER_NAME" && $arResult["PROPERTY_LIST_FULL"][$k]["CODE"] != "NAME"){
                            ...
                            if($arResult["PROPERTY_LIST_FULL"][$k]["CODE"] == "USER_NAME" || $arResult["PROPERTY_LIST_FULL"][$k]["CODE"] == "NAME")
                                $user_name = $item[0];
                    ...
                    if (!empty($arParams["EVENT_MESSAGE_ID"])) {
                        ...
                        $arEM = $rsEM->Fetch();
                        $message_type = $arEM["EVENT_NAME"];
						$id_files = array();

						foreach($arFilesForSend as $key => $value)
						{
							if(!empty($arFilesForSend[$key]["name"]))
							{
								$arImage = array_merge($arFilesForSend[$key], Array("MODULE_ID" => "main"));
								$id_files[] = CFile::SaveFile($arImage, "main");
							}
						}
						/*
						$arImage = array_merge($_FILES["PROPERTY_FILE_756_0"], Array("MODULE_ID" => "main"));
						$id_file = CFile::SaveFile($arImage, "main");
						*/
                        CEvent::Send($message_type, SITE_ID, $arFields, "N", "", $id_files);
                    }
                    else
						CEvent::Send("ADD_ORDER_VISAFREE", SITE_ID, $arFields);
