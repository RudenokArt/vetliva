...
<div class="filter-area white-area">
    <form id="archive-filter" method="GET" action="<?= $APPLICATION->GetCurPageParam("", array("HISTORY_ARCHIVE[DATE_FROM]", "HISTORY_ARCHIVE[DATE_TO]"), false); ?>">
        ...
        <input type="hidden" name="provider_id" value="<?= htmlspecialchars($_REQUEST['provider_id'])?>">
        <input type="hidden" name="service_id" value="<?= htmlspecialchars($_REQUEST['service_id'])?>">
        <fieldset>
                                ...
                                <? if (is_array($v)): ?>
                                    <optgroup label="<?= $arResult["IBLOCK_ELEMENTS"][$k] ?>">
                                        <? foreach ($v as $service_id => $service_name): ?>
                                            <option <? if ($arParams['SERVICE_ID'] == $service_id || $_REQUEST["HISTORY_ARCHIVE"]["SERVICE"] == $service_id): ?>selected<? endif ?> value="<?= $service_id ?>"><?= $service_name ?></option>
                                        <? endforeach ?>
                                    </optgroup>
                                <? else: ?>
