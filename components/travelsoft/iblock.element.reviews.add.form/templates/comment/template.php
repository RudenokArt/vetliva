...
//$this->addExternalJs(SITE_TEMPLATE_PATH . "/js/jquery.magnific-popup.js",true);
            ...
            <?foreach ($arResult["PROPERTY_LIST"] as $pid=>$arProperty):?>
                ...
                <?foreach ($arProperty as $propertyID):?>
                    ...
                    <?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] != "USER" && $arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] != "PHOTO" && $arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] != "ITEM" && $arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] != "RECOMMEND"):?>
                        ...
                        <?else:?>
                            <?if($propertyID == "PREVIEW_TEXT" || $propertyID == "DETAIL_TEXT"):?><?=GetMessage("PROPERTY_".$propertyID)?><?endif?>
                    ...
                    <?endif?>
						...
						switch ($INPUT_TYPE):
							...
							case "N":
								...
								?>
								<input type="<?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] == "USER" || $arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] == "NAME"):?>hidden<?else:?>text<?endif?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="25" value="<?=$value?>" class="field-input review<?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["USER_TYPE"] == "Date"):?> date<?endif?>" <?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] == "ITEM"):?>id="element_item" <?endif?> /><?
...
<script>
    (function ($) {
		$('.show-add-review-popup').click(function(){
			$('#element_item').val($(this).data('element-id'));
		});
