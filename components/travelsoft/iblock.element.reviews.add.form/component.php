...
$email_partners = '#DEFAULT_EMAIL_FROM#';
$arElementId = '';
if(!empty($arParams["ELEMENT_ID"])){
    $db_hotel = CIBlockElement::GetList(Array("SORT"=>"ASC"), Array("IBLOCK_ID"=>PLACEMENTS_IBLOCK_ID, "ACTIVE"=>"Y", "ID"=>$arParams["ELEMENT_ID"]), false, false, Array("ID", "PROPERTY_USER"));
    if($db_hotel->SelectedRowsCount() > 0) {
        while ($ar_hotels = $db_hotel->GetNext()) {
            $arElementId = $ar_hotels["PROPERTY_USER_VALUE"];
        }
    }

    $db_san = CIBlockElement::GetList(Array("SORT"=>"ASC"), Array("IBLOCK_ID"=>SANATORIUM_IBLOCK_ID, "ACTIVE"=>"Y", "ID"=>$arParams["ELEMENT_ID"]), false, false, Array("ID", "PROPERTY_USER"));
    if($db_san->SelectedRowsCount() > 0) {
        while ($ar_san = $db_san->GetNext()) {
            $arElementId = $ar_san["PROPERTY_USER_VALUE"];
        }
    }

    $db_tours = CIBlockElement::GetList(Array("SORT"=>"ASC"), Array("IBLOCK_ID"=>TOUR_IBLOCK_ID, "ACTIVE"=>"Y", "ID"=>$arParams["ELEMENT_ID"]), false, false, Array("ID", "PROPERTY_USER_ID"));
    if($db_tours->SelectedRowsCount() > 0) {
        while ($ar_tours = $db_tours->GetNext()) {
            $arElementId = $ar_tours["PROPERTY_USER_ID_VALUE"];
        }
    }

    if(!empty($arElementId)){
        $rsUser = CUser::GetByID($arElementId);
        $arUser = $rsUser->Fetch();
		if(!empty($arUser["EMAIL"]))
			$email_partners .= ", ".$arUser["EMAIL"];
    }
...
if ($bAllowAccess)
{
	...
	$arResult["PROPERTY_LIST_FULL2"] = array(
		"NAME" => array(
			"PROPERTY_TYPE" => "S",
			"MULTIPLE" => "N",
			"COL_COUNT" => $COL_COUNT,
	...
	while ($arProperty = $rsIBLockPropertyList->GetNext())
	{
		...
		if($arProperty["CODE"] == "USER_NAME") $GLOBALS["ID_FIO_REVIEW"] = $arProperty["ID"];

		// add property to edit-list
		if (in_array($arProperty["ID"], $arParams["PROPERTY_CODES"])) {
		    /*if ($arProperty["PROPERTY_TYPE"] == "L" || $arProperty["PROPERTY_TYPE"] == "F")
                $arResult["PROPERTY_LIST"]["LIST"][] = $arProperty["ID"];
		    else
                $arResult["PROPERTY_LIST"]["INPUT"][] = $arProperty["ID"];*/
		    if ($arProperty["CODE"] == "COUNTRY" || $arProperty["CODE"] == "CITY")
		        $arResult["PROPERTY_LIST"]["PLACE"][] = $arProperty["ID"];
            elseif ($arProperty["CODE"] == "PRICE_QUALITY" || $arProperty["CODE"] == "LOCATION" || $arProperty["CODE"] == "STAFF")
                $arResult["PROPERTY_LIST"]["LIST_1"][] = $arProperty["ID"];
            elseif ($arProperty["CODE"] == "PURITY" || $arProperty["CODE"] == "ROOMS" || $arProperty["CODE"] == "FOOD")
                $arResult["PROPERTY_LIST"]["LIST_2"][] = $arProperty["ID"];
            elseif ($arProperty["CODE"] == "TRANSPORT" || $arProperty["CODE"] == "GUIDE")
                $arResult["PROPERTY_LIST"]["LIST_3"][] = $arProperty["ID"];
		    else
                $arResult["PROPERTY_LIST"][$arProperty["CODE"]][] = $arProperty["ID"];
    ...
    foreach ($arResult["PROPERTY_LIST_FULL2"] as $key => $arr)
    {
        if (in_array($key, $arParams["PROPERTY_CODES"])) $arResult["PROPERTY_LIST"][$key][] = $key;

        $arResult["PROPERTY_LIST_FULL"][$key] = $arr;
    }

    //dm($arResult["PROPERTY_LIST_FULL"]);
    //dm($arResult["PROPERTY_LIST"]);
...
if ($bAllowAccess)
{
	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		foreach ($arParams["PROPERTY_CODES_REQUIRED"] as $key => $propertyID)
		{
			...
			if ($bError)
			{
				$arResult["ERRORS"][] = intval($propertyID) > 0 ? "PROPERTY_".$arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] : "PROPERTY_".$propertyID;
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			else
			{
				...
				$arUpdateValues["NAME"] = $_REQUEST["PROPERTY"][$GLOBALS["ID_FIO_REVIEW"]][0].', '.date($DB->DateFormatToPHP(CSite::GetDateFormat("SHORT")));
				...
				if (!empty($arParams["EVENT_MESSAGE_ID"])) {
					$email = '';
					foreach ($arParams["EVENT_MESSAGE_ID"] as $mail) {
						$email = $mail;
						if((int)$mail == 95){
							$flag_answer = true;
						}
				...
				if($flag_answer && $arParams["IBLOCK_ID"] == 36) {

					$res = CIBlockElement::GetList(Array("SORT"=>"ASC"), Array("IBLOCK_ID"=>$arUpdateValues["IBLOCK_ID"], "ID"=>$arUpdateValues["PROPERTY_VALUES"]["258"]), false, false, Array("IBLOCK_ID","ID","PREVIEW_TEXT","PROPERTY_USER"));
					while($ar_fields = $res->GetNext())
					{
						$preview_text = $ar_fields['~PREVIEW_TEXT'];
						if(!empty($ar_fields["PROPERTY_USER_VALUE"])){
							$rsUser = CUser::GetByID($ar_fields["PROPERTY_USER_VALUE"]);
							$arUser = $rsUser->Fetch();
							if(!empty($arUser["EMAIL"]))
								$email_author = $arUser["EMAIL"];
								//$email_author .= ", ".$arUser["EMAIL"];
						}
					}

					$arLoadProductArray = Array(
						"DETAIL_TEXT"    => $_REQUEST["PROPERTY"]["DETAIL_TEXT"][0],
					);
					if (!$res = $oElement->Update($arUpdateValues["PROPERTY_VALUES"]["258"], $arLoadProductArray))
					{
						$arResult["ERRORS"][] = $oElement->LAST_ERROR;
					}

					/*$prop["ANSWER"] = array('VALUE'=>array('TYPE'=>'TEXT', 'TEXT'=>$_REQUEST["PROPERTY"]["DETAIL_TEXT"][0]));
					CIBlockElement::SetPropertyValuesEx($arUpdateValues["PROPERTY_VALUES"]["258"], false, $prop);*/

					if (empty($arResult["ERRORS"])) {
						$arFields = array(
							"USER_TEXT" => $preview_text,
							"ELEMENT_ID" => $arUpdateValues["PROPERTY_VALUES"]["258"],
							"EMAIL_TO" => $email_author,
							"MESSAGE" => $_REQUEST["PROPERTY"]["DETAIL_TEXT"][0],
						);
						...
						if (!empty($arParams["EVENT_MESSAGE_ID"])) {
							...
							CEvent::Send("ADD_REVIEWS", SITE_ID, $arFields, "N", IntVal($email));
						}
					}

				} else {

					//Получение значения свойтсва "Категория отзыва"
					if (count($arUpdateValues["PROPERTY_VALUES"]["258"]) != 0) {
						$Res = CIBlockElement::GetByID($arParams["ELEMENT_ID"]);
						if ($arItem = $Res->GetNext()) {
							echo $arItem['IBLOCK_ID'] . '<br/>';
							switch ($arItem['IBLOCK_ID']) {
								case 33:
									$arUpdateValues["PROPERTY_VALUES"]["1041"]["VALUE"] = 408;
									break;
								case 8:
									$arUpdateValues["PROPERTY_VALUES"]["1041"]["VALUE"] = 406;
									break;
								case 7:
									$arUpdateValues["PROPERTY_VALUES"]["1041"]["VALUE"] = 407;
									break;
							}
						} else $arResult["ERRORS"][] = 'Ошибка определения id инфоблока';
					} else if ($arUpdateValues["PROPERTY_VALUES"]["853"] == '304') {
						$arUpdateValues["PROPERTY_VALUES"]["1041"]["VALUE"] = 405;
					}
                    ...
                    if (empty($arResult["ERRORS"]) && $arParams["ID"] > 0){

                        $text = '';

                        if(!empty($_REQUEST["PROPERTY"])){
                            foreach ($_REQUEST["PROPERTY"] as $k=>$item) {
                                if($k != "PREVIEW_TEXT" && $arResult["PROPERTY_LIST_FULL"][$k]["CODE"] != "USER_NAME" && $arResult["PROPERTY_LIST_FULL"][$k]["CODE"] != "ITEM" && $arResult["PROPERTY_LIST_FULL"][$k]["CODE"] != "USER"){
                                    if(is_array($item) && !empty($item[0])){
                                        $text .= $arResult["PROPERTY_LIST_FULL"][$k]["NAME"] . ": " . $item[0] ."\n";
                                    }
                                    elseif(($arResult["PROPERTY_LIST_FULL"][$k]["PROPERTY_TYPE"] == "L" || $arResult["PROPERTY_LIST_FULL"][$k]["PROPERTY_TYPE"] == "C") && !is_array($item) && !empty($item))
                                    {
                                        $item = CIBlockPropertyEnum::GetByID($item);
                                        $text .= $arResult["PROPERTY_LIST_FULL"][$k]["NAME"] . ": " . $item["VALUE"] ."\n";
                                    }
                                }
                                if($arResult["PROPERTY_LIST_FULL"][$k]["CODE"] == "USER_NAME")
                                    $user_name = $item[0];
                        ...
                        $arFields = array(
                            ...
                            "EMAIL_TO" => $email_partners,
                            "MESSAGE" => $_REQUEST["PROPERTY"]["PREVIEW_TEXT"][0],
                            "TEXT" => $text
                        );
                        ...
                        {
                            if($arParams["IBLOCK_ID"] == 63)
                            {
                                $email = 90;
                            } else if($arParams["IBLOCK_ID"] == 36)
                                    {
                                        $email = 67;
                                    }
                            CEvent::Send("ADD_REVIEWS", SITE_ID, $arFields, "N", IntVal($email));
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			LocalRedirect($sRedirectUrl."&add_review_form=Y");
			exit();
		}
	}
