...
class TravelsoftHighloadRowAddEdit extends CBitrixComponent {
     ...
     * @return array
     */
    public function processRequest($entity, $row_data, $form_id, $redirect_url = "", $messok = "", $user_field_name = "") {
        ...
        if ($request->isPost() && (strlen($request->getPost('save')) > 0 || strlen($request->getPost('apply')) > 0) &&
            ...
            if (isset($this->arParams['HAS_UF_PICTURES_FIELD']) && $this->arParams['HAS_UF_PICTURES_FIELD'] == 'Y') {
                foreach ($_POST['UF_PICTURES'] as $tmpvalues) 
                    if ($tmpvalues['tmp_name']!='')
                       $_POST['UF_PICTURES_NEW'][] = $tmpvalues;   
                $picturesToDelete = $_POST['UF_PICTURES_del'] ?? [];
                $oldPictures = array_map(function ($picture, $key) use ($picturesToDelete) {
                    return [
                        'name' => '',
                        'type' => '',
                        'tmp_name' => '',
                        'error' => 4,
                        'size' => 0,
                        'del' => isset($picturesToDelete[$key]) && $picturesToDelete[$key] == 'Y' ? 1 : 0,
                        'old_id' => $picture
                    ];
                }, $_POST['UF_PICTURES'], array_keys($_POST['UF_PICTURES']));

                $newPictures = array_map(function ($picture) {
                    return array_merge($picture, ['del' => '', 'old_id' => '', 'tmp_name' => CTempFile::GetAbsoluteRoot() . $picture['tmp_name']]);
                }, $_POST['UF_PICTURES_NEW']);

                $data['UF_PICTURES'] = array_merge(($oldPictures ?: []), ($newPictures ?: []));
            }
            //TODO: uncomment if not enough rights for write
            /*global $USER;
            global $APPLICATION;

            $userGroups = array_map(function(){
                return 'X';
            }, array_flip($USER->GetUserGroupArray()));

            $APPLICATION->SetFileAccessPermission("/ru/index.php", $userGroups);*/
            ...
            if ($ID > 0) {
                ...
                //ADD
                $data[$user_field_name] = $row_data['fields']['UF_USER_ID']['MULTIPLE'] == "N" ? $this->arResult['FILTER_USER_ID'] : (array) $this->arResult['FILTER_USER_ID'];
...
//                ));
            ...
            if ($res->isSuccess()) {
                ...
                travelsoft\booking\Utils::clearTagCacheForSearchOffers([$ID]);
                if (strlen($request->getPost('save')) > 0) {
                    LocalRedirect($redirect_url);
                } else {
                    $redirect_url = "?edit=Y&row_id=" . $ID;
                    if (!empty($this->arParams['FILTER_SERVICE_ID'])) {
                        $redirect_url .= '&provider_id='.$this->arParams['FILTER_SERVICE_ID'];
                    }
                    LocalRedirect($redirect_url);
                }
            } else {
    ...
    public function executeComponent() {
        ...
        try {
            ...
            $this->arResult['FILTER_USER_ID'] = empty($this->arParams['FILTER_SERVICE_ID']) ? $GLOBALS['USER']->GetID() : $this->arParams['FILTER_SERVICE_ID'];
            ...
            if (!empty($this->arResult['ROW_DATA']['row'])) {
                ...
                if (!(in_array($this->arResult['FILTER_USER_ID'], (array) $this->arResult['ROW_DATA']['row'][$this->arParams['USER_LINK_FIELD_NAME']]) || $GLOBALS['USER']->IsAdmin() || $this->arParams['SUPER_USER_EDIT'] === 'Y')) {
                ...
                if ($this->arResult['ROW_DATA']['row']["ID"]) {
                    ...
                    if (in_array($this->arResult['FILTER_USER_ID'], (array) $this->arResult['ROW_DATA']['row'][$this->arParams['USER_LINK_FIELD_NAME']]) || $GLOBALS['USER']->IsAdmin()) {
            ...
            $this->arResult['ERRORS'] = $this->processRequest(
