...
class TravelsoftOrderList extends CBitrixComponent {
    ...
    protected function __getFilter () {
       ...
       if (isset($_REQUEST["order_filter"]["status"]) && (string)$_REQUEST["order_filter"]["status"]!== "unknown"
        ...
        if ($_REQUEST["order_filter"]) {
            ...
            if ($_REQUEST["order_filter"] === "prevent") {
                $filter["tourdate"] = array(
                    "01.01.2016", date("d.m.Y", time()-86400)
                );
            } elseif ($_REQUEST["order_filter"] === "cancelled") {
                $filter["statuses"] = [2,24,25];
            }
    ...
    protected function __setTableColumn () {
        ...
        $this->arResult["IS_AGENT"] = in_array(\Bitrix\Main\Config\Option::get("travelsoft.booking.dev.tools", "agents_group_id"), $GLOBALS["USER"]->GetUserGroupArray());
