    ...
    <div class="row">
        <? if ($arResult["IS_SET_FILTER"] || !empty($arResult["LIST"])): ?>
            <div class="col-lg-6 col-md-6">
                <form method="get" id="order-list-filter" action="<?= $curPage ?>">
                    <div><?= GetMessage("SHOW_BY_FILTER") ?></div>


                    <div class="form-group">
                        <select name="order_filter" class="form-control">
                            <option <? if ($_REQUEST["order_filter"] === "all"): ?>selected=""<? endif ?> value="all"><?= GetMessage("SHOW_ALL_ORDERS") ?></option>
                            <option <? if ($_REQUEST["order_filter"] === "prevent"): ?>selected=""<? endif ?> value="prevent"><?= GetMessage("SHOW_PREVENT_ORDERS") ?></option>
                            <option <? if ($_REQUEST["order_filter"] === "cancelled"): ?>selected=""<? endif ?> value="cancelled"><?= GetMessage("SHOW_CANCELLED_ORDERS") ?></option>
                        </select>
                    </div>

                </form>
                <? if (!empty($arResult["LIST"])): ?>
                    <div ><?= GetMessage("SORT"); ?></div>
