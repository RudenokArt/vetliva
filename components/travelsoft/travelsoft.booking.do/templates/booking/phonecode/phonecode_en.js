...
var countryMaskCache;
var countryMaskRequesting = false;
/**
 * @author Yakov Akulov
...
(function($) {
    $.widget('custom.phonecode', {
        ...
        _loadData : function() {
            var self = this;
            if(!countryMaskCache && !countryMaskRequesting) {
                countryMaskRequesting = $.getJSON('/local/components/travelsoft/travelsoft.booking.do/templates/booking/phonecode/phone-codes.json', {})
                    .done(function(json) {
                        self.datamask = json;
                        countryMaskCache = self.datamask;
                        //self._initSelector();
                    })
                    .fail(function(xhr, status, error) {
                        //alert(status + ' ' + error);
                        self.datamask = countries;
                        countryMaskCache = self.datamask;
                        //self._initSelector();
                    });
            }
            else if(countryMaskCache) {
                this.datamask = countryMaskCache;
                self._initSelector();
            }
            else if(countryMaskRequesting) {
                countryMaskRequesting.done(function(json) {
                    self.datamask = json;
                    countryMaskCache = self.datamask;
                    //self._initSelector();
                });
            ...
            if(!countryCache && !countryRequesting) {
                countryRequesting = $.getJSON('/local/components/travelsoft/travelsoft.booking.do/templates/booking/phonecode/countries_en.json', {})
                    .done(function(json) {
                        self.data = json;
        ...
        _initSelector: function() {
            ...
            var searchLabel = $('<label class="country-phone-search-label">Enter your country</label>');
            ...
            for(var i = 0; i < this.data.length; i++) {
                ...
                var option = $('<div data-phone="'+
                    ...
                    ' class="country-phone-option"><span>+'+ country.ph +'<img src="/local/components/travelsoft/travelsoft.booking.do/templates/booking/phonecode/blank.gif" class="flag flag-'+
                    country.co +
                    '"></span>'+ country.na +'</div>'
                );
                ...
                if(this.options.preferCo && (this.options.preferCo != undefined)) {
                    if(prefCountry == this.options.preferCo) {
                        selected = country;
                        if (this.datamask) {
                            for(var j = 0; j < this.datamask.length; j++) {
                                var selectedmask;
                                if (this.datamask[j].cc==prefCountry.toUpperCase()) {
                                    selectedmask = this.datamask[j].mask;
                                    j=this.datamask.length;
                                }
                            }
                            if (selectedmask) {
                                var tmpnumber = '+'+selected.ph;
                                var tmpmask = selectedmask.replace(tmpnumber, "");
                                tmpmask = tmpmask.replace(/#/g,'9');
                                this.element.mask(tmpmask);
                            }
                        }
                    }
                }
            ...
            if(selected) {
                this.container.find('.country-phone-selected')
                    .html('<img src="/local/components/travelsoft/travelsoft.booking.do/templates/booking/phonecode/blank.gif" class="flag flag-'+ selected.co +'">+'+ selected.ph);
                if (this.datamask) {
                    for(var i = 0; i < this.datamask.length; i++) {
                        var selectedmask;
                        if (this.datamask[i].cc==selected.co.toUpperCase()) {
                            selectedmask = this.datamask[i].mask;
                            i=this.datamask.length;
                        }
                    }
                    if (selectedmask) {
                        var tmpnumber = '+'+selected.ph;
        ...
        setElementSelected: function(el) {
            ...
            if (this.datamask) {
                for(var i = 0; i < this.datamask.length; i++) {
                    var selectedmask;
                    if (this.datamask[i].cc==$(el).data('co').toUpperCase()) {
                        selectedmask = this.datamask[i].mask;
                        i=this.datamask.length;
                    }
                }
                if (selectedmask) {
                    var tmpnumber = '+'+$(el).data('phone');
                    var tmpmask = selectedmask.replace(tmpnumber, "");
                    tmpmask = tmpmask.replace(/#/g,'9');
                    this.element.mask(tmpmask);
                }
