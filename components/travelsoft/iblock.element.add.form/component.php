    ...
    if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"]))) {
                ...
                // for file properties
                    ...
                    foreach ($arPropertyValue as $key => $value) {
                        $arFile = is_numeric($value) ? [
                            ...
                            'size' => 0,
                            'del' => '',
                                ] : array_merge($value, ['error' => 0, 'tmp_name' => CTempFile::GetAbsoluteRoot() . $value['tmp_name']]);
                        $arFile['del'] = isset($_REQUEST['PROPERTY_del'][$propertyID][$key]) && $_REQUEST['PROPERTY_del'][$propertyID][$key] == "Y" ? "Y" : "";
                        $arUpdatePropertyValues[$propertyID][$key] = $arFile;
        ...
        if (empty($arResult["ERRORS"])) {
            if ($arParams["ELEMENT_ASSOC"] == "PROPERTY_ID") {
                $user_id = $USER->GetID();
                // for super user edit
                if ($arParams['SUPER_USER_EDIT'] === 'Y' && $arParams['ID'] > 0) {
                    $property = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arParams['ID'], array("sort" => "asc"), ['ID' => $arParams['ELEMENT_ASSOC_PROPERTY']])->Fetch();
                    
                    if (!empty($property['VALUE'])) {
                        $user_id = $property['VALUE'];
                    }
                }
                $arUpdatePropertyValues[$arParams["ELEMENT_ASSOC_PROPERTY"]] = $user_id;
            }
        ...
        if (empty($arResult["ERRORS"])) {
            ...
            // сохранение цен для платных услуг
            saveServicesPriceInfo($arParams["ID"], $sAction);
...
function saveServicesPriceInfo($element_id, $sAction) {

    if ($_REQUEST["SERVICES_PAID_PROPERTY_ID"] > 0) {

        $pid = intVal($_REQUEST["SERVICES_PAID_PROPERTY_ID"]);

        $ib_element = new CIBlockElement;

        $property_enums = CIBlockPropertyEnum::GetList(Array("DEF" => "DESC", "SORT" => "ASC"), Array("IBLOCK_ID" => SERVICES_PRICES_IBLOCK_ID, "CODE" => "CURRENCY"));
        $arr_currency = array();
        while ($enum_fields = $property_enums->GetNext()) {
            $arr_currency[$enum_fields["VALUE"]] = $enum_fields["ID"];
        }

        $user_name = $GLOBALS["USER"]->GetFullName() . "[" . $GLOBALS["USER"]->GetEmail() . "]";

        if ($sAction === "ADD") {

            foreach ($_REQUEST["PROPERTY"][$pid] as $i => $arr_value) {

                if ($arr_value["VALUE"] > 0 && $_REQUEST["SERVICES_PRICE"][$i] > 0 && isset($_REQUEST["SERVICES_CURRENCY"][$i])) {
                    $ib_element->Add(array(
                        "IBLOCK_ID" => SERVICES_PRICES_IBLOCK_ID,
                        "ACTIVE" => "Y",
                        "NAME" => $user_name,
                        "PROPERTY_VALUES" => array(
                            "SERVICE" => $arr_value["VALUE"],
                            "OBJECT" => array($element_id),
                            "PRICE" => $_REQUEST["SERVICES_PRICE"][$i],
                            "CURRENCY" => $arr_currency[$_REQUEST["SERVICES_CURRENCY"][$i]]
                        )
                    ));
                }
            }
            ...
            foreach ($_REQUEST["PROPERTY"][$pid] as $i => $arr_value) {
                ...
                if ($arr_value["VALUE"] > 0) {
                    ...
                    $arr_element = $ib_element->GetList(false, array(
                                "IBLOCK_ID" => SERVICES_PRICES_IBLOCK_ID,
                                "PROPERTY_SERVICE" => $arr_value["VALUE"],
                                "PROPERTY_OBJECT" => array($element_id)
                                    ),
                                    false,
                                    false,
                                    array(
                    ...
                    if ($arr_element["ID"] > 0) {
                        ...
                        $ib_element->SetPropertyValuesEx($arr_element["ID"], SERVICES_PRICES_IBLOCK_ID, array(
                            "PRICE" => $_REQUEST["SERVICES_PRICE"][$i] > 0 ? $_REQUEST["SERVICES_PRICE"][$i] : "",
                            "CURRENCY" => isset($arr_currency[$_REQUEST["SERVICES_CURRENCY"][$i]]) ? $arr_currency[$_REQUEST["SERVICES_CURRENCY"][$i]] : "",
                        ));
                    } elseif ($_REQUEST["SERVICES_PRICE"][$i] > 0 && isset($_REQUEST["SERVICES_CURRENCY"][$i])) {
...
function getPreparedServicesPriceInfo($id) {

    $db_services_price_info = CIBlockElement::GetList(false, array(
                "IBLOCK_ID" => SERVICES_PRICES_IBLOCK_ID,
                "PROPERTY_OBJECT" => $id
                    ), false, false, array(
                "ID",
    ...
    $arr_services_price_info = array();
    while ($arr_service_price_info = $db_services_price_info->Fetch()) {
        $arr_services_price_info[$arr_service_price_info["PROPERTY_SERVICE_VALUE"]] = array(
            "PRICE" => $arr_service_price_info["PROPERTY_PRICE_VALUE"],
            "CURRENCY" => $arr_service_price_info["PROPERTY_CURRENCY_VALUE"],
        );
    }

    return $arr_services_price_info;
}

function getPreparedServicesIdList($id) {

    $db_element = CIBlockElement::GetByID($id)->GetNextElement();

    $id_list = array();

    if ($db_element) {
        $arProperties = $db_element->GetProperties();
        if (!empty($arProperties["SERVICES_PAID"]["VALUE"])) {
            $id_list = $arProperties["SERVICES_PAID"]["VALUE"];
        }
    }
