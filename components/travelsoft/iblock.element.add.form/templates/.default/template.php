...
$this->addExternalCss(SITE_TEMPLATE_PATH . "/css/jquery.webui-popover.min.css");
$this->addExternalJs(SITE_TEMPLATE_PATH . "/js/webui-popover/jquery.webui-popover.min.js");
$this->addExternalJs("/bitrix/js/main/utils.js");

function getHint($propertyId, $arResult, $arParams) {

    $hint_message = !empty(@$arResult["PROPERTY_LIST_FULL"][$propertyId]["HINT"]) ? $arResult["PROPERTY_LIST_FULL"][$propertyId]["HINT"] : $arParams["CUSTOM_HINT_{$propertyId}"];
    if (!empty($hint_message)) {
        return "&nbsp;<span data-content=\"{$hint_message}\" class=\"hint-block\"><b>?</b></span>";
    }
}
...
$titlesTab = [
    "ru" => array("Описание на русском языке", true),
    "en" => array("Описание на английском языке", false),
    "by" => array("Описание на белорусском языке", false),
];
while ($arLang = $rsLang->Fetch()) {
    $arTabs[$arLang["LID"]] = $titlesTab[$arLang["LID"]];
...
if ($arParams["IS_COPY"] !== "Y" && isset($_REQUEST["CODE"]) && $_REQUEST["CODE"] > 0) {
...
?>
<form id="iblock_add" name="iblock_add" action="<?= $APPLICATION->GetCurPageParam("", ["copy"], false) ?>" method="post" enctype="multipart/form-data">
    <?if ($_REQUEST['is_sanatorium'] === 'Y'):
        // Для редактирования номерного фонда санатория в разделе управления поставщиками?>
    <input name="is_sanatorium" type="hidden" value="Y">
    <?endif?>
            ...
            <div class="tabbable">
                ...
                <? if ($arParams["MAX_FILE_SIZE"] > 0): ?><input type="hidden" name="MAX_FILE_SIZE" value="<?= $arParams["MAX_FILE_SIZE"] ?>" /><? endif ?>
                    ...
                    <div class="tab-content">
                            <? foreach ($arTabs as $tab => $arTab): ?>
                            <div class="tab-pane <? if ($arTab[1]) : ?>active<? endif ?> preload" id="basic-justified-<?= $tab ?>">
                                <?
                                foreach ($arTabsPropertiesId[$tab] as $propertyID):

                                    if ($propertyID . "" === "857") {

                                        echo '<input type="hidden" name="PROPERTY[857][316]" value="316">';
                                        continue;
                                    }
                                    ?>
                                    ...
                                    <div class="form-group">
                                        ...
                                        if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] != "CALC_BY_DAY" &&
                                                ...
                                                $arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] != "ALLOW_WITHOUT_ADULTS"
                                        ):
                                            ?>
                                    ...
                                    ?><? endif ?><? if (in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):
                                                ?><span class="starrequired">*</span><? endif ?></b> <?= getHint($propertyID, $arResult, $arParams) ?></label>
                                        ...
                                        switch ($INPUT_TYPE):
                                            case "USER_TYPE":
                                                if ($arResult['PROPERTY_LIST_FULL'][$propertyID]["CODE"] === "SERVICES_PAID") {
                                                    ...
                                                    $arr_elements = $arr_elements_id = array();
                                                    while ($res = $db_res->Fetch()) {
                                                        $arr_elements[] = $res;
                                                        $arr_elements_id[] = $res["ID"];
                                                    }
                                                    ?>
                                                    <input type="hidden" name="SERVICES_PAID_PROPERTY_ID" value="<?= $propertyID ?>">
                                                    <?
                                                    $arr_services_price_info = $arr_services_id_list = array();
                                                    if ($_REQUEST["CODE"] > 0) {
                                                        $id = htmlspecialchars($_REQUEST["CODE"]);
                                                        $arr_services_id_list = getPreparedServicesIdList($id);
                                                        $arr_services_price_info = getPreparedServicesPriceInfo(htmlspecialchars($id));
                                                    }
                            ...
                            foreach ($arr_elements as $arr_element): ?>
                                                                        <option <? if (in_array($arr_element["ID"], $arr_services_id_list) && !$stop): unset($arr_services_id_list[array_search($arr_element["ID"], $arr_services_id_list)]);
                                    $stop = true;
                                    $current = $arr_element["ID"]; ?>selected=""<? endif ?> value="<?= $arr_element["ID"] ?>"><?= $arr_element["NAME"] ?></option>
                            <? endforeach ?>
                                                                </select>
                                                            </div>
                                                            <div class="col-md-2">
                                                                <input <? if ($current > 0 && $arr_services_price_info[$current]["PRICE"]): ?>value="<?= $arr_services_price_info[$current]["PRICE"] ?>"<? endif ?> name="SERVICES_PRICE[<?= $i ?>]" placeholder="Стоимость" type="text" class="form-control">
                                                            </div>
                                                            <div class="col-md-2">
                                                                <select name="SERVICES_CURRENCY[<?= $i ?>]" class="select">
                                                                    <option <? if ($current > 0 && $arr_services_price_info[$current]["CURRENCY"] === "BYN"): ?>selected=""<? endif ?> value="BYN">BYN</option>
                                                                    <option <? if ($current > 0 && $arr_services_price_info[$current]["CURRENCY"] === "RUB"): ?>selected=""<? endif ?> value="RUB">RUB</option>
                                                                    <option <? if ($current > 0 && $arr_services_price_info[$current]["CURRENCY"] === "EUR"): ?>selected=""<? endif ?> value="EUR">EUR</option>
                                                                    <option <? if ($current > 0 && $arr_services_price_info[$current]["CURRENCY"] === "USD"): ?>selected=""<? endif ?> value="USD">USD</option>
                                                                </select>
                                                            </div>
                                                        </div>
                            <?
                        ...
                        ?>
                                        ...
                                        if (elements.length) {
                                            ...
                                            $("#add-select-<?= $propertyID ?>").on("click", function () {
                                                ...
                                                $(this).data("select-num", (Number(selectNum) + 1));
                                                    ...
                                                    if ($arResult['PROPERTY_LIST_FULL'][$propertyID]["PROPERTY_TYPE"] == "E" && ($arResult['PROPERTY_LIST_FULL'][$propertyID]["MULTIPLE"] === "Y")) {
                                                        ?>
                            ...
                            ?>
                                            ...
                                            case "HTML":
                                                $fields = [
                                                    "HD_DESC", "NDAYS", "PRICE_INCLUDE"
                                                ];
                                                $disallow_for_edit = in_array($arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"], $fields) && $arParams["IS_COPY"] !== "Y" &&
                                                        $arr_element_properties["NOT_ALLOW_EDIT"]["VALUE"] === "Y";
                                                if ($disallow_for_edit) {
                                                    ShowError("Поле \"{$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]}\" недоступно для редактирования. Пожалуйста, обратитесь в службу поддержки.");
                                                    for ($i = 0; $i < $inputNum; $i++) {
                                                        ?><input name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" type="hidden" value='<?= $arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["~VALUE"]["TEXT"] ?>'><div><?= $arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["~VALUE"]["TEXT"] ?></div><?
                                                    }
                                                } else {
                                                    for ($i = 0; $i < $inputNum; $i++) {
                                                        $LHE = new CHTMLEditor;
                                                        ...
                                                        $LHE->Show(array(
                                                            ...
                                                            'content' => !is_array($arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["~VALUE"]) ? $arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["~VALUE"] : $arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["~VALUE"]["TEXT"],
                                                            'width' => '100%',
                                                            'minBodyWidth' => 350,
                                                            'normalBodyWidth' => 555,
                                                            'height' => "75",
                                            ...
                                            case "N":
                                                for ($i = 0; $i < $inputNum; $i++) {
                                                    ...
                                                    ?>
                                                    <input class="form-control" type="text" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" size="25" value="<?= $value ?>" /><? if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["USER_TYPE"] == "DateTime"):
                                                        ?><?
                                                        $APPLICATION->IncludeComponent(
                                            ...
                                            case "F":
                                                $inputName = [];
                                                foreach ($arResult["ELEMENT_PROPERTIES"][$propertyID] as $key => $value) {
                                                    $inputName["PROPERTY[{$propertyID}][" . ($arResult["ELEMENT_PROPERTIES"][$propertyID][$key]["VALUE_ID"] ? $arResult["ELEMENT_PROPERTIES"][$propertyID][$key]["VALUE_ID"] : $key) . "]"] = $value['VALUE'];
                                                    //$inputName["PROPERTY[{$propertyID}][{$key}]"] = $value['VALUE'];
                                                }

                                                $html = \Bitrix\Main\UI\FileInput::createInstance([
                                                            'name' => "PROPERTY[{$propertyID}][#IND#]",
                                                            'description' => false,
                                                            'upload' => true,
                                                            'allowUpload' => 'A',
                                                            'medialib' => false,
                                        ...
                                        ?> />
                                                            ...
                                                            <? else: ?>
                                                                    <label><b> <? if (intval($propertyID) > 0): ?> <?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] ?><? else: ?> <?= !empty($arParams["CUSTOM_TITLE_" . $propertyID]) ? $arParams["CUSTOM_TITLE_" . $propertyID] : GetMessage("IBLOCK_FIELD_" . $propertyID)
                                                                    ?><? endif ?><? if (in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):
                                                                    ?><span class="starrequired">*</span><? endif ?></b> <?= getHint($propertyID, $arResult, $arParams) ?>
                                                                    </label><br />
                                                                <? endif ?>
                                                            </div>
                                                                <?
...
<? endif ?>
<? //if(isset($arr_element_properties["NOT_ALLOW_EDIT"]) || $arr_element_properties["NOT_ALLOW_EDIT"]["VALUE"] !== "Y" || $arParams["IS_COPY"] === "Y"): ?>
            <div class="mt-20">
...
#bx-html-editor-PROPERTY2220, 
#bx-html-editor-PROPERTY2221,
#bx-html-editor-PROPERTY2222,
#bx-html-editor-PROPERTY2223,
#bx-html-editor-PROPERTY2224,
#bx-html-editor-PROPERTY2225,
#bx-html-editor-PROPERTY2226,
#bx-html-editor-PROPERTY2227,
#bx-html-editor-PROPERTY2228 {display: none;}
...
<script>
    ((function ($) {
		...
		$("#bx-html-editor-PROPERTY2220").parent('.form-group').addClass('program-parent'); 
		$("#bx-html-editor-PROPERTY2220").parent().append($('<div class="button-area mt-10 text-right"><button class="nextstep btn btn-primary">Добавить</button></div>'));
	  
		var activeStep = 0;
			$(".nextstep").on("click", function () {
    		  $('.program-parent .bx-html-editor').eq(activeStep++).show();
              return false;
            });

        setTimeout(function(){
            $(".program-parent .bx-html-editor input[type=hidden]").each(function () {
                if($(this).val().length) {
        ...
        $(".hint-block").each(function () {
            var $this = $(this);
            $this.webuiPopover({content: $this.data("content"), trigger: 'hover', placement: "right"});
        });
        ...
        function autosave() {
            var form = document.getElementById("iblock_add");
    ...
    window.onload = (event) => {
        setTimeout(() => $('.tab-content .tab-pane').removeClass('preload'), 1000);
