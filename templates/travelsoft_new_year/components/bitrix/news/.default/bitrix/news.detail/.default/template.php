...
<section class="hl-features detail-cn" id="hl-features">
    <div class="row">
        <div class="col-lg-3 detail-sidebar">
            <div class="scroll-heading">
            <?= showItem("SERVICES", $arResult["MENU_ITEM"]); ?>
            </div>
        </div>
        <div class="col-lg-9 hl-features-cn">
            ...
            <? if (!empty($arResult["DISPLAY_PROPERTIES"]["MED_SERVICES"]["VALUE"])): ?>
                <div class="featured-service">
                        ...
                        <?foreach ($arResult["MED_SERVICES_GROUP"] as $sectionId => $arServices):?>
                            <div class="list-service-section">
                                <?if ($arResult["MED_SERVICES_SECTIONS"][$sectionId]["PICTURE"]["SRC"]):?>
                                <div class="icon-service" style="float:left; top:2px"><img src="<?= $arResult["MED_SERVICES_SECTIONS"][$sectionId]["PICTURE"]["SRC"]?>"></div>
                                <?endif?>
                                <h4><?= $arResult["MED_SERVICES_SECTIONS"][$sectionId]["TITLE"]?></h4>
                            </div>
                            <ul class="service-accmd">
                                <?foreach ($arServices as $serviceId => $arData):?>
                                <li><div><img src="/local/templates/travelsoft/images/icon-check.png" alt=""></div><?= $arData["TITLE"]?> <?if ($arData["PAID"]):?><a data-content="<?= GetMessage('SERVICE_PAID') ?>">(<i class="fa fa-dollar"></i>)</a><?endif?></li>
...
<!-- End Hotel Features -->
<!-- Details-Policies -->
<section class="details-policies detail-cn" id="block_hd_descroom">
    <div class="row">
        <div class="col-lg-3 detail-sidebar">
            <div class="scroll-heading">
<?= showItem("HD_DESCROOM", $arResult["MENU_ITEM"]); ?>
            </div>
        </div>
        <div class="col-lg-9 details-policies-cn">
            ...
            <? if (!empty($arResult["PROPERTIES"]["HD_DESCROOM" . POSTFIX_PROPERTY]["VALUE"])): ?>
                <a class="color" name="block_<?= mb_strtolower($arResult["PROPERTIES"]["HD_DESCROOM"]["CODE"]) ?>"></a>
                <div class="policies-item">
            ...
            <? if (!empty($arResult["PROPERTIES"]["HD_DESCMEAL" . POSTFIX_PROPERTY]["VALUE"])): ?>
                <a class="color" name="block_<?= mb_strtolower($arResult["PROPERTIES"]["HD_DESCMEAL"]["CODE"]) ?>"></a>
                <div class="policies-item">
                    <h3><?= $arResult["PROPERTIES"]["HD_DESCMEAL" . POSTFIX_PROPERTY]["NAME"] ?></a></h3>
            ...
            <? if (!empty($arResult["PROPERTIES"]["HD_DESCSPORT" . POSTFIX_PROPERTY]["VALUE"])): ?>
                <a name="block_<?= mb_strtolower($arResult["PROPERTIES"]["HD_DESCSPORT"]["CODE"]) ?>"></a>
                <div class="policies-item">
...
<? endif ?>
            ...
            <!-- Details Policies Item -->
                ...
                <a name="block_<?= mb_strtolower($arResult["PROPERTIES"]["YOUTUBE"]["CODE"]) ?>"></a>
                <div class="policies-item">
...
<section class="about-area detail-cn" id="about-area">
    <div class="row">
        ...
        <div class="col-lg-9">
                ...
                <a  name="block_<?= mb_strtolower($arResult["PROPERTIES"]["MAP"]["CODE"]) ?>"></a>
...
-->
<?$this->SetViewTarget('reviews_detail');?>
