...
if ($arParams["MAKE_PRICING"] && \Bitrix\Main\Loader::includeModule("travelsoft.booking.dev.tools") && \Bitrix\Main\Loader::includeModule("highloadblock") && \Bitrix\Main\Loader::includeModule("travelsoft.currency")) {
	...
	$is_filterdop_request = !empty($arParams["__FILTERTYPE_REQUEST"]);//наличие фильтра дополнительного по типу объекта
    ...
    //составляем дополнительное условие для фильтрации по типам объектов

	$filter_attr_types=array();	
	if ($is_filterdop_request)
	{
		$attr_types =$arParams["__FILTERTYPE_REQUEST"];	

		//получаем список ID достопримечательностей по выбранному фильтру
		$arFilter_dop = array("IBLOCK_ID" => ATTRACTION_IBLOCK_ID, "ACTIVE" => "Y","PROPERTY_TYPE"=>$attr_types);
		$list=$APPLICATION->IncludeComponent("travelsoft:travelsoft.iblock.getlist.byfilter", "", Array(
        "CACHE_TIME" => 3600,
        "CACHE_TYPE" => "A",
        "CNT" => 999999,
        "FILTER" => $arFilter_dop,
		...
		if (!empty($list["ELEMENT_ID"])) $filter_attr_types =  array("PROPERTY_SIGHTS" =>$list["ELEMENT_ID"]);
    ...
    $arElements = $APPLICATION->IncludeComponent("travelsoft:travelsoft.iblock.getlist.byfilter", "", Array(
        ...
        "CACHE_TYPE" => "A",
        "CNT" => 999999,
        "FILTER" => array_merge($arFilter, $complex_logic,$filter_attr_types),
        "FILTER_NAME" => $arParams["FILTER_NAME"],
        "ORDER" => array($arParams["SORT_BY1"] => $arParams["ORDER_BY1"]),
        "RETURN_RESULT" => "Y",
        "SORT" => false,
	...
	//если есть фильтр по типу достопримечательностей, записываем ID
	if ($is_filterdop_request) {
        $_res_ids = $arElements["ELEMENT_ID"];
        $GLOBALS[$arParams["FILTER_NAME"]]["ID"] = !empty($_res_ids) ? $_res_ids : array(-1);
...
<?if ($is_mobile):?><a href="<?=$APPLICATION->GetCurPageParam("del_filter=Y", array("set_filter", "del_filter"), false) ?>" onclick="$('.magnificbutton').magnificPopup('close');" class="mobile-reset-filtr sorting" ><?=GetMessage('FILTER_RESET') ?></a><?endif;?>
    ...
    $APPLICATION->IncludeComponent(
		...
		"__FILTERTYPE_REQUEST" => $arParams["__FILTERTYPE_REQUEST"],	
		"IBLOCK_ID_TYPE_SIGHTS" => $arParams["IBLOCK_ID_TYPE_SIGHTS"],
		"IBLOCK_ID_TYPE_SIGHTS" => $arParams["IBLOCK_ID_TYPE_SIGHTS"],
...
<?
$APPLICATION->IncludeComponent(
        "travelsoft:travelsoft.news.list", "excursions-list-new", Array(
    "IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
    "IBLOCK_ID" => $arParams["IBLOCK_ID"],
