...
?>
		<li class="socserv-wdth">
			<!--<a id="bx_socserv_icon_<?/*=$service["ID"]*/?>" class="btn btn-primary btn-socserv" href="javascript:void(0)" onclick="<?/*=htmlspecialcharsbx($onclick)*/?>" title="<?/*=htmlspecialcharsbx($service["NAME"])*/?>"><?/*=GetMessage('BTN_SOCSERV')*/?><?/*=htmlspecialcharsbx($service["NAME"])*/?></a>-->
            <a id="bx_socserv_icon_<?=$service["ID"]?>" class="<?=$service["ICON"]?> bx-authform-social-icon socservice-line" href="javascript:void(0)" onclick="<?=htmlspecialcharsbx($onclick)?>" title="<?=htmlspecialcharsbx($service["NAME"])?>"><?=GetMessage('BTN_TEXT')[$service['ID']]?></a>
