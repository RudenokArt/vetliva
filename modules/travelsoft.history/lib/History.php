...
class History {
    ...
    public function clear (array $parameters = null) {
        ...
        $parameters["select"] = array("ID", "UF_ACTION");
        include $_SERVER["DOCUMENT_ROOT"] . "/local/php_interface/include/classes/Logger.php";
        $logger = new rest\Logger;
        
        do{
            $deleted_pool_id = array();
            $arResult = $this->get($parameters);
            foreach ($arResult as $arFields) {
                $dataClass::delete($arFields["ID"]);
                $deleted_pool_id[] = $arFields["ID"] . " " . $arFields["UF_ACTION"];
            }
            $logger->write(implode(" | ", $deleted_pool_id));
