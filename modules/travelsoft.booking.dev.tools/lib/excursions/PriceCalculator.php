...
class PriceCalculator extends CommonPriceCalculator {
                ...
                foreach ($arGroupedData as $unixdate => $arSubGroupedData) {
                    ...
                    foreach ($arSubGroupedData as $rateId => $arPriceTypesData) {
                        ...
                        if (isset($arPriceTypesData[4])) {
                            ...
                            // ФИЛЬТРАЦИЯ ТАРИФОВ, КОТОРЫЕ НЕ ПОДХОДЯТ ПО ЗАПРОСУ
                            // ИСПОЛЬЗУЕТСЯ ЗДЕСЬ, А НЕ ПРИ ФОРМИРОВАНИИ DataProvider,
                            // ТАК КАК НЕОБХОДИМА ОБРАТНАЯ СОВМЕСТИМОСТЬ СО СТАРОЙ ВЕРСИЕЙ
                            if ($arRates[$rateId][0]['UF_FOR_PLACE'] == 0) {
                                ...
                                // МНОГОДНЕВНЫЕ ТУРЫ
                                if (
                                        $this->_dataProvider->request->adults > $arRates[$rateId][0]['UF_ADULTS'] ||
                                        $this->_dataProvider->request->children > $arRates[$rateId][0]['UF_CHILDREN'] ||
                                        $this->_dataProvider->request->children + $this->_dataProvider->request->adults > $arRates[$rateId][0]['UF_ADD_PLACES'] + $arRates[$rateId][0]['UF_MAIN_PLACES']
                                ) {
                                ...
                                // ОДНОДНЕВНЫЕ ТУРЫ
                                $arAllocate = $this->allocate(1000, 0);
                            }
    ...
    public static function applyDiscount($arResult = null, array $arRates = []) {
